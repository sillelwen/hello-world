...
$rowid = intval($_GET['lineid']);
$showstatus = hasNamedRight($_SESSION['userid'], 'viewstatus', $connect);

if(($res = getUserRegisterRowByID($connect, $_SESSION['userid'], $rowid)) && $res->num_rows>0)
{
	$row=$res->fetch_assoc();
...
{	
	//сначала проверим введённые данные
	if($_POST['receptionDate'] < date('Y-m-d'))
	{
		$message .= '<p class="message">Нельзя установить дату забора на '.dateForPrint($_POST['receptionDate']).', поскольку эта дата уже прошла.';
	}
	elseif(isDateClosed($connect, $_POST['receptionDate']))
	{
		$message .= '<p class="message">Нельзя установить дату забора на '.dateForPrint($_POST['receptionDate']).', поскольку этот день уже закрыт.';
	}
	else
	{
		...
		if(in_array('direction',$editable) && isset($_POST['direction']))
			if($_POST['direction']!=$row['direction'])
			{
				$set[] = "direction='".prepStr($_POST['direction'])."'";
				$changes['direction'] = ['name'=>'Направление', 'newvalue'=>getDirectionName($_POST['direction']), 'oldvalue'=>getDirectionName($row['direction'])];
				$row['direction'] = $_POST['direction'];
			}
		if(in_array('takePapers',$editable) && (isset($_POST['takePapers']) || $row['takePapers']))
			if($_POST['takePapers']!=$row['takePapers'])
			{
				$set[] = "takePapers=".intval($_POST['takePapers']);
				$changes['takePapers'] = ['name'=>'Забрать документы', 'newvalue'=>boolForPrint(boolval($_POST['takePapers'])), 'oldvalue'=>boolForPrint(boolval($row['takePapers']))];
				$row['takePapers'] = $_POST['takePapers'];
			}
		if(in_array('needNotification',$editable) && (isset($_POST['needNotification']) || $row['needNotification']))
			if($_POST['needNotification']!=$row['needNotification'])
			{
				$set[] = "needNotification=".intval($_POST['needNotification']);
				$changes['needNotification'] = ['name'=>'Требуется уведомление', 'newvalue'=>boolForPrint(boolval($_POST['needNotification'])), 'oldvalue'=>boolForPrint(boolval($row['needNotification']))];
				$row['needNotification'] = $_POST['needNotification'];
			}
		if(count($set)>0)
		{
			...
			if($connect->query($query))
			{
				...
				logevent($connect, EVENT_TYPE_ORDERDATA, 'editregisterline', 'Изменены данные отправления', $rowid, serialize($details));
...
?>
	...
	<body onload="swapDirection()">
		...
		<p><a href="listregisters.php">Список отправлений</a>&nbsp;&gt;&nbsp;Редактирование отправления</p>
				...
				<table>
					...
					<tbody>
						...
						<tr>
							...
							<td><input name="receptionDate" type="date" placeholder="ГГГГ-ММ-ДД" min="<?=date('Y-m-d')?>" max="<?=$date->format('Y-m-d')?>" <?php if(isset($receptionDate)) echo "value=\"$receptionDate\""; if(!in_array('receptionDate',$editable)) echo ' readonly';?> required /></td>
						</tr>
						<tr>
...
?>
							...
							<td>
								<input type="date" name="deliveryDate" placeholder="ГГГГ-ММ-ДД" min="<?=$deldate->format('Y-m-d')?>" <?php if(isset($deliveryDate)) echo "value=\"$deliveryDate\""; if(!in_array('deliveryDate',$editable)) echo ' readonly';?> required />
<?php
	}
?>
						</tr>
...
?>
						...
						<tr>
							<td>Направление</td>
							<td>
								<input type="radio" name="direction" id="direction1" onchange="swapDirection();" value="<?=DIRECTION_DELIVERY?>"<?=$direction==DIRECTION_DELIVERY ? ' checked' : ''?><?=!in_array('direction',$editable) ? ' disabled' : ''?>/><label for="direction1"> <?=getDirectionName(DIRECTION_DELIVERY)?></label><br/>
								<input type="radio" name="direction" id="direction2" onchange="swapDirection();" value="<?=DIRECTION_RECEPTION?>"<?=$direction==DIRECTION_RECEPTION ? ' checked' : ''?><?=!in_array('direction',$editable) ? ' disabled' : ''?>/><label for="direction2"> <?=getDirectionName(DIRECTION_RECEPTION)?></label><br/>
								<input type="radio" name="direction" id="direction3" onchange="swapDirection();" value="<?=DIRECTION_BOTH?>"<?=$direction==DIRECTION_BOTH ? ' checked' : ''?><?=!in_array('direction',$editable) ? ' disabled' : ''?>/><label for="direction3"> <?=getDirectionName(DIRECTION_BOTH)?></label>
							</td>
						</tr>
					</tbody>
				</table>
				...
				<table width="100%">
					...
					<tbody>
						...
						<tr>
							<td><input type="checkbox" name="takePapers" id="takePapers" value="1"<?=$takePapers?" checked":""?><?php if(!in_array('takePapers',$editable)) echo ' disabled';?>/><label for="takePapers"> Забрать документы</label></td>
						</tr>
						<tr>
							<td><input type="checkbox" name="needNotification" id="needNotification" value="1"<?=$needNotification?" checked":""?><?php if(!in_array('needNotification',$editable)) echo ' disabled';?>/><label for="needNotification"> Требуется уведомление</label></td>
						</tr>
					</tbody>
				</table>
			</div>
